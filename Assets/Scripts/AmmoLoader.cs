...
public class AmmoLoader : MonoBehaviour
{
    ...
    void Awake(){
        stack=new Stack(gameObject,3,1,50);
    ...
    internal void AddDropper(GameObject _dropper){
       ...
       // Debug.Log(collectors.IndexOf(_gameObject));
        ...
        while (true){
            if(Wallet.instance.arrow<1 || droppers.Count==0 || health<1){
                stopWorkerToDesk();
                yield return null;
                ...
                CharController activeDropper=droppers[0].GetComponent<CharController>();
                    ...
                    GameObject takenItem=pooler.SpawnFromPool("Arrow",activeDropper.transform.position,Quaternion.identity);
                    if(!takenItem){
                        stopWorkerToDesk();
                        yield return null;
